...
#define DATEPICKER_MAXDATE [[[NSCalendar currentCalendar] components:NSYearCalendarUnit fromDate:[NSDate date]] year]
#define DATEPICKER_MINDATE 1915
...
//                [monthArray addObject:[NSString stringWithFormat:@"%d月",i+1]];
                [monthArray addObject:[NSString stringWithFormat:@"%02zd",i+1]];
...
//        [yearArray addObject:[NSString stringWithFormat:@"%d年",i]];
    ...
    if (self.maxLimitDate) {
        ...
        self.maxLimitDate = [self dateFromString:[NSString stringWithFormat:@"%ld12312359",DATEPICKER_MAXDATE-1] withFormat:@"yyyyMMddHHmm"];
        maxDateModel = [[WMDatepicker_DateModel alloc]initWithDate:self.maxLimitDate];
    }
    //最小限制
...
- (NSInteger)findMinuteIndex:(NSString *)minute
{
    for ( int i = 0; i<60; i++) {
        ...
        {
            minute = [NSString stringWithFormat:@"%zd",[minute integerValue] +1] ;
        }
...
//        [dayArray addObject:[NSString stringWithFormat:@"%02d日",i]];
        [dayArray addObject:[NSString stringWithFormat:@"%02zd",i]];
    }
}
